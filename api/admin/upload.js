// api/admin/upload.js
// Vercel serverless function: server-mediated upload to Supabase Storage
// Зависимости: @supabase/supabase-js, busboy
...
export default async function handler(req, res) {
  ...
  // Ожидаем Authorization: Bearer <access_token>
  const authHeader = req.headers.authorization || '';
  const token = authHeader.split(' ')[1];
  if (!token) return res.status(401).json({ error: 'No token provided' });

  // Проверка, что пользователь — админ (таблица public.admins)
  let uid = null;
  try {
    // supabase-js v2: auth.getUser(token)
    const { data: userData, error: userErr } = await supabaseAdmin.auth.getUser(token);
    if (userErr || !userData?.user) {
      // Попытка через legacy /auth/v1/user
  ...
  busboy.on('finish', async () => {
    try {
      ...
      // Загружаем файл в Storage как admin
      ...
      // Получаем public URL (если bucket public). Если bucket private — используйте storage_path для генерации signed URL при отдаче.
      const { publicURL } = supabaseAdmin.storage.from(BUCKET).getPublicUrl(path);

      // Вставляем запись в factory_media
